...
import cn.rongcloud.voicequickdemo.ApiFun;
import cn.rongcloud.voicequickdemo.QuickEventListener;
...
public class ApiFunDialogHelper {
     ...
     * @param resultBack
     */
    public void showSelectDialog(Activity activity, String roomId, String title, IResultBack<Account> resultBack) {
        if (null == dialog || !dialog.enable()) {
            dialog = new QDialog(activity,
                    new DialogInterface.OnDismissListener() {
                        @Override
        ...
        RecyclerView refresh = new RecyclerView(dialog.getContext());
        IAdapte adapter = new RcySAdapter<Account, RcyHolder>(dialog.getContext(), R.layout.layout_item_selector) {
            @Override
            public void convert(RcyHolder holder, Account accout, int position) {
                holder.setText(R.id.selector_name, accout.getUserName());
        ...
        PKApi.getRoomMembers(roomId, new com.kit.wapper.IResultBack<List<Account>>() {
            ...
            public void onResult(List<Account> result) {
                result.remove(AccoutManager.getAccount(AccoutManager.getCurrentId()));
                adapter.setData(result, true);
            }
        });
    ...
    public void showSelectDialog(Activity activity, String title, IResultBack<Account> resultBack, boolean all) {
        if (null == dialog || !dialog.enable()) {
            dialog = new QDialog(activity,
                    new DialogInterface.OnDismissListener() {
                        @Override
        ...
        if (all) {
            ...
            List<Account> accounts = AccoutManager.getAccounts();
            List<Account> onlines = new ArrayList<>();
