...
public class TestPkActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onDestroy() {
        super.onDestroy();
        cn.rongcloud.voice.pk.PKStateManager.get().unInit();
    }
    ...
    private void initData() {
        ...
        join();
        cn.rongcloud.voice.pk.PKStateManager.get().init(voiceRoomBean.getRoomId(), pkVew, new IPKState.VRStateListener() {
            @Override
            public void onPkStart() {
                cn.rongcloud.voice.pk.PKStateManager.get().enterPkWithAnimation(voice_room, pkVew, 200);
            }

            @Override
            public void onPkStop() {
                cn.rongcloud.voice.pk.PKStateManager.get().quitPkWithAnimation(pkVew, voice_room, 200);
            }
    ...
    public void onClick(View view) {
        ...
        if (R.id.leave == id) {
            ...
            cn.rongcloud.voice.pk.PKStateManager.get().refreshPKGiftRank();
            ...
            cn.rongcloud.voice.pk.PKStateManager.get().quitPK(activity);
            ...
            if (state == IEventHelp.Type.PK_INVITE) {
                cn.rongcloud.voice.pk.PKStateManager.get().cancelPkInvitation(activity, new IResultBack<Boolean>() {
                    @Override
                ...
                cn.rongcloud.voice.pk.PKStateManager.get().sendPkInvitation(activity, new IResultBack<Boolean>() {
                    @Override
                    public void onResult(Boolean aBoolean) {
